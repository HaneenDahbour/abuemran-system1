...
# ── Unlinked Employee Names (for user-creation dropdown) ─────
...
async def get_unlinked_employee_names(user=Depends(get_current_user)):
    ...
    rows = await pool.fetch("""
        ...
        WHERE employee_user_id IS NULL AND employee_name IS NOT NULL
        UNION
        SELECT DISTINCT employee_name FROM employee_advances
        WHERE user_id IS NULL AND employee_name IS NOT NULL
