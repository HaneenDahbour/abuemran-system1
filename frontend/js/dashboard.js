...
function openUserModal() {
  ...
  openModal(`
    ...
    <div class="form-group">
      <label class="form-label">اختر من الموظفين الحاليين (اختياري)</label>
      <select class="form-select" id="nu_existing_emp" onchange="
        const v=this.value;
        if(v){document.getElementById('nu_name').value=v;}
      ">
        <option value="">— أو اكتب اسماً جديداً —</option>
      </select>
    </div>
    <div class="form-group">
  ...
  // تحميل أسماء الموظفين غير المرتبطين في القائمة المنسدلة
  API.getUnlinkedEmployeeNames().then(names => {
    const sel = document.getElementById('nu_existing_emp');
    ...
    return `
              <tr>
                ...
                <td style="display:flex;gap:4px;flex-wrap:wrap;min-width:80px">
                  <button class="btn btn-ghost btn-sm" onclick="saveCategoryInvestment('${categoryId}', '${s.investor_id}')" title="حفظ رأس المال">💾</button>
...
async function openInvestorModal(investorId = null) {
  ...
  try {
    const [cats, invData] = await Promise.all([
      API.getWarehouseCategories(),
      investorId ? API.getWarehouseInvestor(investorId) : Promise.resolve(null),
    ]);
    categories = cats || [];
    if (invData) {
      inv = invData.investor || {};
      existingInvestments = invData.investments || [];
    }
  } catch (e) {
  ...
  openModal(`
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:12px">
      <div class="form-group" style="margin:0">
        <label class="form-label">الاسم *</label>
        <input class="form-input" id="wi_name" value="${escHtml(inv?.name || '')}" placeholder="اسم المستثمر">
      </div>
      <div class="form-group" style="margin:0">
        <label class="form-label">رقم الهاتف</label>
        <input class="form-input" id="wi_phone" value="${escHtml(inv?.phone || '')}" placeholder="05xxxxxxxx">
      </div>
    </div>
...
async function openInvestorDetailsModal(investorId) {
  ...
  openModal(`
    <div class="modal-header">
      <div class="modal-title">📊 ${escHtml(inv.name)} — المساهمات والأرباح</div>
      <button class="modal-close" onclick="closeModal()">✕</button>
    </div>
